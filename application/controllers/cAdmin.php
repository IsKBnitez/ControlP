...
class CAdmin extends CI_Controller
{
 ...
 function __construct()
  ...
  public function Index()
  {
    ...
    $this->load->view('Frontend/vAdmin');
  ...
  public function consultaruuss()
  {
    if ($this->input->is_ajax_request()) {
      $datos = $this->mAdmin->consultaruuss();
  ...
  public function consultar()
  {
    if ($this->input->is_ajax_request()) {
      ...
      $datos = $this->mAdmin->consultar($buscar);
      echo json_encode($datos);
    }
    else {
      show_404();
  ...
  public function guardar()
  {
    ...
    $usuario = $this->mAdmin->valordelid($this->input->post("uss"));

    $config =
    [
      "upload_path" => "./img/alumnos",
    ...
    if($this->upload->do_upload('fotos'))
    {
      ...
      $datos=array(
        ...
        'email_alum' =>$email,
        'id_us' =>$usuario);
      if($this->mAdmin->guardar($datos)==true){
...
public function modificar()
{
  ...
  $usuario = $this->mAdmin->valordelid($this->input->post("uss2"));
  ...
  if($this->upload->do_upload('fotos2'))
  {
    ...
    $datos=array(
      ...
      'email_alum' =>$email,
      'id_us' =>$usuario);
    if($this->mAdmin->modificar($id,$datos)==true){
...
public function eliminar(){
  if ($this->input->is_ajax_request()) {
    ...
    $registro = $this->mAdmin->Nimagen($id);
    unlink("./img/alumnos/".$registro);
    if ($this->mAdmin->eliminar($id)==true) {
